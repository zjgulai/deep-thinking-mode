...
export function createFakeRouterDom({ payload, payloadNodePresent = true, reducedMotion = false, clipboardWrite } = {}) {
  ...
  const routeCards = [
    ["diagnosis::intent", "core"],
    ["planning::intent", "auxiliary"],
    ["decision::intent", "auxiliary"],
    ["creative::planning", "core"],
    ["diagnosis::planning", "auxiliary"]
  ].map(([routeKey, initialKind]) => node("route-key", routeKey, {
    hidden: true,
    attributes: { "data-initial-kind": initialKind }
  }));
  ...
  const clarifyButtons = ["diagnosis", "planning", "decision", "creative", "research"]
    .map((id) => node("clarify-option", id, { hidden: true }));
  const safetyPanels = ["immediate_personal_danger", "medical_diagnosis_or_treatment"]
  ...
  const root = new FakeRouterNode({
    children: [
      ...(payloadNodePresent ? [payloadNode] : []),
      form,
      input,
      live,
      hint,
