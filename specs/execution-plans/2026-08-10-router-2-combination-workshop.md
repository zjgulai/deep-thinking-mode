...
实现顺序固定：NFKC → Unicode 小写 → 标点/空白折叠 → 保留中文/字母/数字 → 同时生成紧凑文本。每个正向短语只计一次；先减负向、最低归零；最佳示例二元组 Jaccard 小于 0.22 不奖励，否则奖励 `Math.min(6, Math.floor(similarity * 6))`；快捷意图只给同 ID +8。
